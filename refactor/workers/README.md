...
- In contrast, every web worker runs on its own thread
  - always concurrently
  - potentially in parallel, depending on the number of available logical processors
- `password.html` and `sha1.js` demonstrate how to:
  - create web workers, and
  - communicate via message passing
- ⚠️ Using web workers requires the project to be served by Node.js `http-server` or similar (see `projects/xkcd`)
- `benoit.html` could benefit from web workers because it is computationally expensive
  - Split the computation among `n` web workers, depending on your hardware
  - Ctrl+Shift+Esc, `Performance` tab, `Logical processors`
